<template>
    ...
    <!-- change mobile number  -->
      <Dialog v-model:visible="official_mobile_change" modal class="custum_dialog_width" :draggable="false">
          <div class="text-center">
              <p class="main-title"> {{ $t('Glopal.Change_mobile_number') }} </p>
          </div>
          <form ref="responsiblePhone" @submit.prevent="submitData">
      ...
      <validationCode ref="official_validation" @changeOfficialPassword="changeOfficialPassword"/>
...
import validationCode from '@/components/Dialogs/myDataDialogs/centerDialogs/responsiblevalidationCode.vue';
...
export default {
  components: {
    ...
    methods: {
        ...
        async submitData() {
        this.loading = true;
        const fd = new FormData(this.$refs.responsiblePhone);
        const config = {
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
        };
        fd.append("country_code", this.selectedCountry.key);
        await axios
            .post(`provider/responsible-phone/update`, fd, config)
            .then((res) => {
            if (res.data.key == "success") {
                this.showSuccess(res.data.msg);
                this.official_mobile_change = false;
                this.$refs.official_validation.activation_code = true;
            } else {
