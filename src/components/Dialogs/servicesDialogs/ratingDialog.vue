<template>
    ...
    <Dialog v-model:visible="services_rating_dialog" modal class="custum_dialog_width" :draggable="false">
        <div class="text-center">
            <div class="main-text">
              <p class="main-title"> {{$t('Glopal.customer_reviews_service')}} </p>
            </div>
            ...
            <div class="static-rate mb-4">
                ...
                <div class="rate-state m-0" v-if="!loader">
                  <div class="clinic_evaluation">
                    <star-rating :star-points="[23,2, 14,17, 0,19, 10,34, 7,50, 23,43, 38,50, 36,34, 46,19, 31,17]" :show-rating="false" :increment="0.5" active-color="#FFB300" :star-size="20" :read-only="true" :animate="true" :rounded-corners="true" :rtl="false" :rating="main_rate" :border-width="2"></star-rating>
                  </div>
                </div>
                  ...
                  <div class="rate-info">
                    ...
                    <div class="foot-rate">
                      ...
                      <div class="rate-state sm">
                        <star-rating :star-points="[23,2, 14,17, 0,19, 10,34, 7,50, 23,43, 38,50, 36,34, 46,19, 31,17]" :show-rating="false" :increment="0.5" active-color="#FFB300" :star-size="12" :read-only="true" :animate="true" :rounded-corners="true" :rtl="false" :rating="rate.rate" :border-width="2"></star-rating>
                      </div>
                    </div>
                  </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    async serviceRate(id) {
      const config = {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
      };
      // this.$emit("rateService", id);
      // console.log(id, "cnnnnhild");
        this.loader = true;
        await axios
          .get(`provider/services/${id}/rates`, config)
          .then((res) => {
