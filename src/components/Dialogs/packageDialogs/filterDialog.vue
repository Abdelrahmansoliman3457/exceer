<template>
    ...
    <Dialog v-model:visible="package_filtering_dialog" modal class="custum_dialog_width" :draggable="false">
            ...
            <form ref="filterForm" @submit.prevent="submitData">
                ...
                <div class="radios form-group">
                    ...
                    <!-- <label class="custom-radio">
                        <input type="radio" name="price" value="radio_2" class="d-none">
                        <span class="mark"></span>
                ...
                <div class="radios form-group">
                    <label  class="label"> {{$t('Glopal.gender')}} </label>
                    <div class="d-flex gap-3">
                    <label class="custom-radio">
...
export default {
  components: {
    ...
    data() {
        return {
            categories: [],
            checkedIds: [],
            package_filtering_dialog: false,
            price: null
        }
  },
  ...
  methods: {
    ...
    cancellation() {
        this.package_filtering_dialog = false
    },
   ...
   async get_classifications() {
    ...
    async submitData() {
     ...
     const config = {
      ...
      await axios
        .post(`provider/packages/filter`, fd, config)
        .then((res) => {
          if (res.data.key == "success") {
            this.showSuccess(res.data.msg);
            this.package_filtering_dialog = false;
        const sortedpackage = res.data.data.packages.sort((a, b) => b.price - a.price);
            this.$emit('packagefiltered', sortedpackage);
            this.checkedIds = [];
            this.price = null;
          } else {
