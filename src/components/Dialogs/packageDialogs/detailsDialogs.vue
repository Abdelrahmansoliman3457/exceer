<template>
    ...
    <Dialog v-model:visible="package_details" modal class="custum_dialog_width" :draggable="false">
        <div class="text-center" v-if="!loader">
            ...
            <div class="main-text mb-4">
                <p class="main-title">{{ $t('Glopal.package_description') }}</p>
                <p class="main-disc">{{ detailsData.description }}</p>
            </div>

            <div class="main-text mb-4">
                <p class="main-title">{{ $t('Glopal.package_terms') }}</p>
...
export default {
  ...
  methods: {
    async getdetails(detailsId) {
        ...
        await axios.get(`provider/packages/${detailsId}/show`, config)
        .then((res) => {
